...
export default function DealDetails() {
  ...
  const handleBuyNow = async () => {
    ...
    navigate(`/checkout/${deal.slug}?qty=${quantity}&dealId=${deal._id}`);
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {deal.relatedDeals?.length > 0 && (
              <div>
                <h3 className="font-bold text-gray-900 text-lg mb-4">{t('deal.related')}</h3>
              ...
              <div className="card p-6">
                ...
                >
                  {purchasing ? <span className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> :
                    <><ShoppingCart size={20} />{t('deal.buy_now')} — {formatCurrency(deal.discountedPrice * quantity)}</>}
                </button>
