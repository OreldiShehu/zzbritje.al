...
import { Search, Filter, Ban, CheckCircle, User, Shield, Building, ChevronDown } from 'lucide-react';
...
export default function AdminUsers() {
  ...
  const blockMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries(['admin', 'users']); toast.success('Statusi u ndryshua!'); },
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-100">Menaxhimi i Përdoruesve</h1>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-700">
                {['Përdoruesi', 'Email', 'Roli', 'Anëtar Që', 'Statusi', 'Veprime'].map((h) => (
                ...
                <tr key={u._id} className="border-b border-gray-700/50 hover:bg-gray-700/30 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={`flex items-center gap-1 text-xs font-medium w-fit px-2 py-1 rounded-full ${u.isActive ? 'bg-green-900/40 text-green-400' : 'bg-red-900/40 text-red-400'}`}>
                      {u.isActive ? <><CheckCircle size={12} />Aktiv</> : <><Ban size={12} />Bllokuar</>}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <button
