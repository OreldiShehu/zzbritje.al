...
import { Search, Building, CheckCircle, XCircle, Eye, Clock, ExternalLink } from 'lucide-react';
...
const STATUS_LABEL = { pending: 'Pending', under_review: 'Nën Shqyrtim', verified: 'Verifikuar', rejected: 'Refuzuar' };
    ...
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-gray-800 rounded-3xl p-6 max-w-lg w-full border border-gray-700 max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-2 text-sm mb-5">
          {[['Kategoria', business.category?.name], ['Qyteti', business.city], ['Email', business.owner?.email], ['Telefon', business.phone], ['Website', business.website], ['Plan', business.subscriptionPlan]].map(([k, v]) => v && (
          ...
          {business.description && <div className="flex gap-3"><span className="text-gray-500 w-24 flex-shrink-0">Përshkrimi</span><span className="text-gray-300 text-xs">{business.description}</span></div>}
        </div>

        {business.documents?.length > 0 && (
          <div className="mb-5">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Dokumentet</p>
        ...
        {mode === 'reject' && (
          <div className="mb-4">
            <label className="block text-xs text-gray-400 mb-1.5">Arsyeja e Refuzimit *</label>
            <textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={3} className="w-full bg-gray-700 border border-gray-600 rounded-xl px-3 py-2 text-sm text-gray-200 resize-none focus:outline-none focus:border-red-500" placeholder="Shpjegoni arsyen..." />
          </div>
        )}
...
export default function AdminBusinesses() {
  ...
  const verifyMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries(['admin', 'businesses']); toast.success('Biznesi u verifikua!'); setSelected(null); },
    onError: () => toast.error('Ndodhi një gabim.'),
  });

  const rejectMutation = useMutation({
    mutationFn: ({ id, reason }) => api.patch(`/admin/businesses/${id}/reject`, { reason }),
    onSuccess: () => { qc.invalidateQueries(['admin', 'businesses']); toast.success('Aplikimi u refuzua.'); setSelected(null); },
    onError: () => toast.error('Ndodhi një gabim.'),
  });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-100">Bizneset</h1>
          <p className="text-gray-500 text-sm">{data?.pagination?.total || 0} gjithsej</p>
        </div>
      </div>
      ...
      <div className="bg-gray-800 rounded-2xl p-4 mb-5 border border-gray-700 flex gap-3">
        <div className="relative flex-1">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Kërko biznes..."
        ...
        <div className="flex gap-2 flex-wrap">
          {[{ v: '', l: 'Të gjitha' }, { v: 'pending', l: 'Pending' }, { v: 'under_review', l: 'Shqyrtim' }, { v: 'verified', l: 'Verifikuar' }, { v: 'rejected', l: 'Refuzuar' }].map(({ v, l }) => (
      ...
      {isLoading ? (
            ...
            <div key={b._id} className="bg-gray-800 rounded-2xl p-4 border border-gray-700 flex items-center gap-4 hover:border-gray-600 transition-all">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0 ${STATUS_BADGE[b.verificationStatus] || 'bg-gray-700 text-gray-400'}`}>{STATUS_LABEL[b.verificationStatus]}</span>
                </div>
        ...
        <div className="text-center py-20 bg-gray-800 rounded-2xl border border-gray-700">
          ...
          <p className="text-gray-400">Nuk ka biznese me këtë status</p>
        </div>
      )}
